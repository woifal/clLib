...
adminUserObj["password"] = "asdffoobar";
var adminDBSession;
			...
			RESTHandler.updateEntity( 
				...
				function(resultObj, responseStream) {
					...
					responseStream.send(JSON.stringify({
						result: "email sent to " + userDetails.username
					}));
			...
			if(req.params["verificationToken"] == dbVerificationToken) {
				...
				RESTHandler.updateEntity( 
					...
					function(resultObj, responseStream) {
						...
						// send token to user..
						util.log("Sending email..");
						var emailBody = "Your new password for '" + userDetails["username"] + "'(at '" + userDetails["email"] + "') is '" + req.params["password"] + "'.";
						mailHandler.send({
						...
						responseStream.send(JSON.stringify({
							result: "pwd changed + email sent to " + userDetails.username
						}));
...
/*
clLib.server.email.replacePlaceholders = function(template, templateVars) {
	var result = template;

	$.each(templateVars, function(key, val) {
		result = template.replace("[" + key + "]", val);
	});
...
/*
clLib.server.email.send = function(entityInstance, template, templateVars) {
	var mailTo = entityInstance["username"];
	var mailBody = clLib.server.email.replacePlaceHolders(template, templateVars);
	var mailSubject = "Verification required to change your Kurtl p";
	
	var emailParams = {
		to: mailTo,
		body: mailBody,
		subject: mailSubject,
		cc: "wolfgang.dietersdorfer+ccKurtl@gmail.com"
	};
	
	alert(
		"Sending email " + JSON.stringify(emailParams)
	);
	...
	return ["sent mail to >" + emailParams["to"] + "<"];
};
...
server.get('/test', function(req, res) {
	...
	/*
	RESTHandler.updateEntity("users", userDetails["_id"], 
		{
			"verificationToken": verificationToken, 
			"verified": false
		}
	...
	EmailHandler.send(
		userDetails, 
		clLib.email.templates.verificationEmail,
		{
